...
SWEEP_SPEND_CAP_USD = 5.0
SWEEP_TIMEOUT_HOURS = 4
SWEEP_TRAIN_SAMPLE_CAP = 384
SWEEP_TRAIN_TOKEN_CAP = 393_216
SWEEP_EVAL_SAMPLE_CAP = 128
SWEEP_EVAL_TOKEN_CAP = 262_144
SWEEP_MATCHED_EVAL_SAMPLE_CAP = 64
SWEEP_MATCHED_EVAL_TOKEN_CAP = 131_072
...
def build_interval_sweep_preflight(
    ...
    *,
    ...
    return {
        ...
        "mode": "modal_interval_eval_sweep",
        ...
        "uses_showcase_output_dir": False,
        "volume": config.runtime["modal_volume"],
        "volume_output_root": str(DEFAULT_MODAL_SWEEP_ROOT),
        "forbidden_full_run_output_root": "/posttrain/esme-instruct-sft-showcase-full",
        ...
        "acceptance": {
            "gate": "heldout response loss must move down versus step 0 for at least one sane arm",
            "metric": "eval/matched/response_loss",
            "baseline_step": 0,
        },
        "dependency_pins": {"modal": MODAL_CLIENT_VERSION},
        "launch_blockers": blockers,
        "modal_sweep_command": sweep_launch_command(
            config.config_path, timeout_hours=timeout_hours, gpu=str(config.runtime["selected_gpu"])
        ),
    }


def sweep_launch_blockers(
    config: SFTLaunchConfig, *, timeout_hours: int, modal_gpu: str
) -> list[str]:
    blockers: list[str] = []
    ...
    if timeout_cost > SWEEP_SPEND_CAP_USD:
        blockers.append("sweep timeout cost ceiling exceeds the approved sweep spend cap")
    max_sequence_tokens = int(config.budgets["max_sequence_tokens"])
    projected_tokens = sum(
        arm.planned_token_upper_bound(max_sequence_tokens=max_sequence_tokens) for arm in SWEEP_ARMS
    )
    ...
    if projected_cost > SWEEP_SPEND_CAP_USD:
        blockers.append("projected interval sweep cost exceeds the approved sweep spend cap")
    if str(DEFAULT_MODAL_SWEEP_ROOT).endswith("showcase-full"):
        blockers.append("sweep output root must not target the showcase full-run directory")
...
def sweep_launch_command(config_path: Path, *, timeout_hours: int, gpu: str) -> str:
    return (
        f"SFT_MODAL_GPU='{gpu}' SFT_SWEEP_TIMEOUT_HOURS={timeout_hours} "
        f"uv run --with modal=={MODAL_CLIENT_VERSION} modal run "
        f"scripts/modal_instruct_sft.py --config {config_path} --modal-sweep --approved --json"
    )


def run_interval_eval_sweep(
    config: SFTLaunchConfig,
    *,
    output_root: Path,
    ...
    write_json(
        ...
        {
            "mode": "bounded_interval_eval_sweep",
...
def _run_sweep_arm(
    ...
    *,
    ...
    write_json(
        ...
        {
            "mode": "bounded_interval_eval_sweep_arm",
            "arm": arm.to_dict(max_sequence_tokens=int(config.budgets["max_sequence_tokens"])),
            ...
            "approval": {
                ...
                "condition": "bounded real-Esme interval-eval SFT sweep only; not full/showcase",
            },
        },
    )
        ...
        SFTTrainerConfig(
            ...
            wandb=WandbConfig(
                ...
                tags=(
                    ...
                    "run-type:interval-sweep",
                    "dataset-mix:smol-smoltalk-80-tulu-personas-20",
...
def _eval_splits(
    matched_eval_reports: dict[str, Any], no_robots_examples: tuple[Any, ...]
) -> tuple[EvalSplit, ...]:
    return (
        EvalSplit(
            "smol-smoltalk",
    ...
    *, status: str, best_arm: dict[str, Any] | None, evidence_uri: str
    ...
    if best_arm is None:
        return {
            ...
            "blocker": (
                "heldout response loss did not move down versus step 0 for any completed "
                "sane sweep arm"
            ),
        }
    return {
