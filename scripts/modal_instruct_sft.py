...
from esme_posttrain.sft.full_instruct import mirror_launch_evidence, run_full_instruct_sft
...
from esme_posttrain.sft.smoke_instruct import refresh_manifest_files, run_cpu_fixture_sft
from esme_posttrain.sft.sweep_instruct import (
    DEFAULT_MODAL_SWEEP_ROOT,
    SWEEP_TIMEOUT_HOURS,
    build_interval_sweep_preflight,
    run_interval_eval_sweep,
    sweep_launch_blockers,
)
...
MODAL_APP_NAME = "esme-posttrain-esme-instruct-sft"
MODAL_SMOKE_OUTPUT_STEM = "esme-instruct-sft-recipe-modal-smoke"
DEFAULT_MODAL_FULL_OUTPUT_STEM = "esme-instruct-sft-showcase-full"
MODAL_FULL_OUTPUT_STEM = os.environ.get(
    "SFT_MODAL_FULL_OUTPUT_STEM",
    DEFAULT_MODAL_FULL_OUTPUT_STEM,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="modal_instruct_sft.py",
        description="Validate and launch the approval-gated Esme Instruct SFT smoke.",
    )
    parser.add_argument("--config", required=True, type=Path, help="SFT config JSON path.")
    ...
    parser.add_argument(
        "--local-cpu-smoke",
        action="store_true",
        help="Run the no-spend local CPU fixture and write evidence.",
    )
    parser.add_argument(
        "--modal-smoke",
        action="store_true",
        help="Launch the capped Modal smoke after approval.",
    )
    parser.add_argument(
        "--modal-sweep",
        action="store_true",
        help="Launch the approved bounded real-data interval-eval Modal sweep.",
    )
    parser.add_argument(
        "--throughput-probe",
        action="store_true",
        help="Run the approved bounded high-end GPU throughput probe.",
    )
    parser.add_argument(
        "--full-run",
        action="store_true",
        help="Launch the approved, capped full Modal SFT run.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
...
def launch(argv: list[str] | None = None) -> int:
    ...
    if mode_count > 1:
        print("instruct SFT launch failed: choose exactly one launch mode", file=sys.stderr)
        return 2
    if args.full_run and args.modal_sweep:
        print(
            "instruct SFT launch failed: --full-run and --modal-sweep are separate modes",
            file=sys.stderr,
        )
        return 2
    if args.resume and not args.full_run:
        print("instruct SFT launch failed: --resume requires --full-run", file=sys.stderr)
        return 2
    try:
        config = load_sft_config(args.config)
    except LaunchError as error:
        print(f"instruct SFT launch failed: {error}", file=sys.stderr)
        return 2
    try:
    ...
    if args.local_cpu_smoke:
        payload = run_cpu_fixture_sft(config)
        print(_format_payload(payload, json_output=args.json))
        return 0
    if args.full_run:
    ...
    if not args.modal_smoke:
        print(_format_payload(build_sft_dry_run(config), json_output=args.json))
        return 0
    if not args.approved:
        print(
            f"instruct SFT Modal smoke refused: pass {LAUNCH_APPROVAL_FLAG} after chat approval",
            file=sys.stderr,
        )
        return 2
    ...
    if blockers:
        ...
        return 2
    if modal is None:
        print("instruct SFT Modal smoke failed: modal is not installed", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"instruct SFT Modal smoke failed before durable success: {error}", file=sys.stderr)
        return 2
...
def _launch_modal_sweep(config: Any, *, approved: bool, json_output: bool) -> int:
    preflight = build_interval_sweep_preflight(
        config,
        timeout_hours=SFT_SWEEP_TIMEOUT_HOURS,
        modal_gpu=SFT_MODAL_GPU,
    )
    if not approved:
        ...
        return 2
    blockers = sweep_launch_blockers(
        config,
        timeout_hours=SFT_SWEEP_TIMEOUT_HOURS,
        modal_gpu=SFT_MODAL_GPU,
    )
    ...
    if modal is None or run_modal_sweep is None:
        print("instruct SFT Modal sweep failed: modal is not installed", file=sys.stderr)
        return 2
    try:
        ...
        print(f"instruct SFT Modal sweep failed before durable success: {error}", file=sys.stderr)
        return 2
...
def _launch_throughput_probe(config: Any, *, approved: bool, json_output: bool) -> int:
    preflight = build_throughput_probe_preflight(
        config,
        modal_gpu=SFT_MODAL_GPU,
        timeout_hours=SFT_PROBE_TIMEOUT_HOURS,
    )
    if not approved:
        ...
        print(_format_payload(payload, json_output=json_output))
        return 2
    blockers = throughput_probe_blockers(
        modal_gpu=SFT_MODAL_GPU,
        timeout_hours=SFT_PROBE_TIMEOUT_HOURS,
    )
    bundle_blocker = _base_bundle_blocker()
    ...
    if modal is None or run_modal_throughput_probe is None:
        print("instruct SFT throughput probe failed: modal is not installed", file=sys.stderr)
        return 2
    try:
        ...
        print(
            f"instruct SFT throughput probe failed before durable success: {error}", file=sys.stderr
        )
        return 2
    print(_format_payload(result, json_output=json_output))
    return 0
...
def _launch_full_run(
    ...
    if modal is None or run_modal_full_sft is None:
        print("instruct SFT full run failed: modal is not installed", file=sys.stderr)
        return 2
    try:
    ...
    payload = {
        ...
        "modal_call_id": call_id,
        "full_launch_command": full_launch_command,
        "resume_launch_command": _resume_launch_command(config, output_stem),
        "resume": resume,
    ...
    receipt = mirror_launch_evidence(
        REPO_ROOT / config.output_dir.parent / "full-run-launch",
        payload,
    )
    payload["local_launch_receipt"] = str(receipt)
    print(_format_payload(payload, json_output=json_output))
    return 0
...
def _run_modal_smoke_body(
    ...
    if estimated_cost > float(config.runtime["smoke_max_cost_usd"]):
        raise RuntimeError("Modal smoke exceeded the approved $2 cap")
...
def _run_modal_sweep_body(
    ...
    *,
    ...
    if blockers:
        raise RuntimeError("interval sweep refused inside Modal: " + "; ".join(blockers))
    return run_interval_eval_sweep(
        config,
...
def _run_modal_throughput_probe_body(
    ...
    *,
    ...
    if blockers:
        raise RuntimeError("throughput probe refused inside Modal: " + "; ".join(blockers))
    return run_throughput_probe(
        config,
...
def _run_modal_full_sft_body(
    ...
    *,
    ...
    if blockers:
        raise RuntimeError("full SFT refused inside Modal: " + "; ".join(blockers))
    output_dir = VOLUME_MOUNT / output_stem
    return run_full_instruct_sft(
        config,
...
def _resume_launch_command(config: Any, output_stem: str) -> str:
    return f"{_full_launch_command(config, output_stem)} --resume"
